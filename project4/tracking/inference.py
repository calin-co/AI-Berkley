...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if(noisyDistance is None) :
            prison_cell = self.getJailPosition()
            allPossible[prison_cell] = 1
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        #store the post beliefs here
        allPossible = util.Counter()
        for p in self.legalPositions :
            new_pos_distr = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for new_pos, prob in new_pos_distr.items():
                allPossible[new_pos] += prob * self.beliefs[p]
    ...
    def initializeUniformly(self, gameState):
        ...
        while (num_parts > 0):
            for p in legal_pos:
                if(num_parts == 0):
                    break
                stored_particles.append(p)
                num_parts -= 1

        #return(stored_particles)
        self.particles = stored_particles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        legal_pos = self.legalPositions
        all_poss_positions = list(itertools.product(legal_pos, repeat = 2))

        '''
        for i in xrange(num_parts):
            particle = random.choice(all_poss_positions)
            self.particles.append(particle)
        '''
        while (num_parts > 0):
            for p in all_poss_positions:
                if(num_parts == 0):
                    break
