...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        nextState_bestQvalue = self.computeValueFromQValues(nextState)
        self.Qvalue[(state, action)] = (1 - self.alpha) * self.Qvalue[(state, action)] + \
                                       self.alpha * (reward + self.discount * nextState_bestQvalue)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for key,value in feats.iteritems():
            self.weights[key] += self.alpha * difference * feats[key]
